...
def graph(key):
    ...
    plt.scatter(standard_concs[key], standard_abs[key], marker=".", label=f"{key}検量用サンプル", color='black')
    plt.plot(x, y_pred, label=f'Fit: R$^{{{2}}}$={r2:.3f}', color='blue')
    plt.scatter(x_obj, objective_abs[key], color='red', label=f"濃度:{x_obj:.3f}, 吸光度:{objective_abs[key]:.3f}")
